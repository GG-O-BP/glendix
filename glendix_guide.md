...
└── glendix
    ...
    ├── binding     ← 외부 React 컴포넌트 (gleam.toml 또는 bindings.json)
...
| `.mpk` Pluggable 위젯 (gleam.toml) | `glendix/widget` + `glendix/interop` | Switch, Badge |
| `.mpk` Classic (Dojo) 위젯 (gleam.toml) | `glendix/classic` | CameraWidget |
...
> `bindings.json`도 폴백으로 동작합니다. gleam.toml에 `[tools.glendix.bindings]`가 있으면 bindings.json은 무시됩니다.
...
Marketplace TUI(`gleam run -m glendix/marketplace`)에서 다운로드하면 gleam.toml에 자동 추가됩니다.
...
`glendix/mendix` 모듈의 `cx` 함수로 CSS 클래스를 조건부 조합합니다:
...
|--------|------|
| `gleam build` | 컴파일 |
| `gleam run -m glendix/install` | 의존성 + TOML 위젯 다운로드 + 바인딩 + 위젯 .gleam 생성 |
...
| `gleam run -m glendix/marketplace` | Marketplace 위젯 다운로드 (인터랙티브, gleam.toml 자동 기록) |
...
| 외부 React 컴포넌트용 `.mjs` 직접 작성 | `gleam.toml [tools.glendix.bindings]` 또는 `bindings.json` + `glendix/binding` 사용 |
| `.mpk` 위젯용 `.mjs` 직접 작성 | `gleam.toml [tools.glendix.widgets.*]` + `glendix/widget` 사용 |
...
| 위젯 바인딩 미생성 | `widget_ffi.mjs` 스텁 상태 | `gleam.toml [tools.glendix.widgets.*]` 설정 후 install |
