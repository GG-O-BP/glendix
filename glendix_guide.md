...
└── glendix
    ...
    ├── widget      ← .mpk 위젯 컴포넌트 (gleam.toml 자동 다운로드 또는 widgets/)
...
`gleam run -m glendix/install` 실행 시 `build/widgets/`에 캐시하고 바인딩을 자동 생성합니다.
Marketplace TUI(`gleam run -m glendix/marketplace`)에서 다운로드하면 gleam.toml에 자동 추가됩니다.

**방법 B: `.mpk`를 `widgets/`에 수동 배치** → `gleam run -m glendix/install`
...
| `.mpk` 위젯용 `.mjs` 직접 작성 | `gleam.toml [tools.glendix.widgets.*]` 또는 `widgets/` + `glendix/widget` 사용 |
...
| 위젯 바인딩 미생성 | `widget_ffi.mjs` 스텁 상태 | `gleam.toml [tools.glendix.widgets.*]` 설정 또는 `widgets/`에 `.mpk` 배치 후 install |
