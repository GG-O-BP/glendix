...
- `.mpk` XML의 `<property>` 정의를 부모 위젯 XML에 `<propertyGroup caption="{위젯명}">` 으로 자동 주입 (동일 caption이 이미 있으면 건너뜀)
...
| 명령어 | 설명 |
|--------|------|
| `gleam run -m glendix/install` | 의존성 설치 + 바인딩 생성 + 위젯 바인딩 생성 + 위젯 속성 XML 주입 (PM 자동 감지) |
...
glendix/
  ...
  cmd_ffi.mjs               ← Node.js child_process + fs + ZIP 파싱 FFI + 바인딩/위젯 바인딩 생성 + 위젯 속성 XML 주입
  build.gleam               ← 빌드 스크립트
