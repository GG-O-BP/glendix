...
function configuredBindings(bindings) {
  return bindings.toArray().map(
    ([moduleName, exports, initializer, retry]) => ({
      moduleName,
...
function importedName(exportName) {
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(exportName)
    ? exportName
    : JSON.stringify(exportName);
}

function renderBindingsSource(configuredEntries) {
  const modules = configuredEntries.filter(
    entry => entry.exports.length > 0 || entry.initializer,
  );
  ...
  modules.forEach((entry, moduleIndex) => {
    const names = uniqueNames([
      ...entry.exports,
      ...(entry.initializer ? [entry.initializer] : []),
    ]);
    const aliases = new Map(
      names.map((name, exportIndex) => [
        name,
        `__glendix_module_${moduleIndex}_export_${exportIndex}`,
      ]),
    );
    imports.push(
      `import { ${names.map(name => `${importedName(name)} as ${aliases.get(name)}`).join(", ")} } from ${JSON.stringify(entry.moduleName)};`,
    );

    const exportedValues = uniqueNames(entry.exports).map(
      name => `${JSON.stringify(name)}: ${aliases.get(name)}`,
    );
    const initialization = entry.initializer
      ? `{ exportName: ${JSON.stringify(entry.initializer)}, retry: ${JSON.stringify(entry.retry)}, run: () => ${aliases.get(entry.initializer)}() }`
      : "null";
    entries.push(
      `  ${JSON.stringify(entry.moduleName)}: { name: ${JSON.stringify(entry.moduleName)}, exports: { ${exportedValues.join(", ")} }, initialization: ${initialization} }`,
    );
  });

  return (
    `// Generated by glendix/install. Do not edit manually.\n` +
    `// Static imports keep configured modules and their WebAssembly assets visible to Rollup.\n` +
    `import { Ok, Error as GleamError } from "../gleam.mjs";\n` +
    ...
    `  if (!mod) return new GleamError("No generated binding exists for module: " + name + ". Check [tools.glendix.bindings] in gleam.toml.");\n` +
    `  return new Ok(mod);\n` +
    `}\n\n` +
    `export function resolve(mod, name) {\n` +
    `  const value = mod.exports[name];\n` +
    `  if (value === undefined) return new GleamError("Module export is not configured: " + name);\n` +
    `  return new Ok(value);\n` +
    `}\n\n` +
    ...
    `  if (!initialization) return new GleamError("Module does not require asynchronous initialization: " + mod.name);\n` +
    `  try {\n` +
    `    const promise = initialization.run();\n` +
    `    if (!promise || typeof promise.then !== "function") {\n` +
    `      return new GleamError("Initializer must return a Promise: " + initialization.exportName);\n` +
    `    }\n` +
...
export function render_binding_source(bindings) {
  return renderBindingsSource(configuredBindings(bindings)) ?? "";
}

function generateBindingsOrThrow(bindings) {
  const configuredEntries = configuredBindings(bindings);
  const content = renderBindingsSource(configuredEntries);
  ...
  if (written > 0) {
    const moduleNames = configuredEntries.map(entry => entry.moduleName).join(", ");
    console.log(`Bindings generated: ${moduleNames}`);
  }
}
